...
class SupabaseHandlerASC:
    ...
    def __init__(self):
        ...
        self.source_name = "Medicare ASC Addenda"
    ...
    def _validate_and_prepare_records(self, records: List[Dict]) -> List[Dict]:
        ...
        Uses rel_date from data processor (extracted from column name).
        """
        ...
        for record in records:
            prepared = prepare_record_for_insertion(
                ...
                has_geozip=False  # ASC data doesn't have geozip
            )
            if prepared:
    ...
    def insert_records(self, records: List[Dict]) -> dict:
        ...
        # For ASC, geozip is NULL, so it's effectively (source, code, release_date)
